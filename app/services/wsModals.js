...
angular.module('wsModals', [])
       .service('modals', ['auth', '$http', '$q', function(auth, $http, $q) {
                        ...
                        '<div class="form-group">'+
                            ...
                            '<div class="col-sm-6">'+
                                '<div class="input-group">'+
                                    '<span class="input-group-addon">'+auth.user+':</span>'+
        ...
        var createModal = $('<div>').kbasePrompt({
                ...
                controls : [{
                    ...
                    callback : function(e, $prompt) {
                        ...
                        if (s_ws.length > 1) {
                            if (s_ws[0] == auth.user) {
                                var proj = auth.user+':'+s_ws[1];
                            }  else {
                                error = 'Only your username ('+auth.user+') may be used before a colon';
                            }
                        } else {
                            var name = auth.user+':'+new_ws_name
                        }
                        ...
                        if (error) {
                            ...
                            var prom = $http.rpc('ws', 'create_workspace', args);
                            prom.then(function(){
                                if (params.submit_cb) params.submit_cb();
                                kbui.notify('Created workspace: '+new_ws_name, 'success');
                                $prompt.closePrompt();
                            }).catch(function(e) {
                ...
                controls : [{
                    ...
                    callback : function(e, $prompt) {
                        ...
                        if (s_ws.length > 1) {
                            ...
                            var new_ws_name = auth.user+':'+new_ws_name
                        }
                        ...
                        if (error) {
                            ...
                            var prom = $http.rpc('ws', 'clone_workspace', p);
                            prom.then(function(){
                                $prompt.closePrompt();
                                if (params.submit_cb) params.submit_cb();
                                $prompt.closePrompt();
        ...
        var deleteModal = $('<div>').kbasePrompt({
                ...
                controls : [{
                    ...
                    callback : function(e, $prompt) {
                        ...
                        var prom = $http.rpc('ws', 'delete_workspace', p);
                        prom.then(function(){
                            params.submit_cb()
                            $prompt.closePrompt();
                        }).catch(function() {
    ...
    this.manageWS = function(params) {
        ...
        var manage_modal = $('<div>').kbasePrompt({
                title : 'Manage Workspace '+
                    (auth.user ? '<a class="btn btn-primary btn-xs btn-edit">Edit <span class="glyphicon glyphicon-pencil"></span></a>' : ''),
                body : content,
                modalClass : '',
                controls : [{
                    name: 'Close',
                    ...
                    callback : function(e, $prompt) { //Fixme: yyyeeeahh.
                        ...
                        prom.then(function() {
                            ...
                            // saving description
                            var params = {workspace: ws_name, description: d}
                            var p1 = $http.rpc('ws', 'set_workspace_description', params)
                            p1.then(function() {
                                var new_perm = $('.btn-global-perm option:selected').val();
                                // saving global perm
                                var p1 = $http.rpc('ws', 'set_global_permission', {workspace: ws_name,
        ...
        var copyWS = $('<button class="btn btn-link pull-left">Copy Workspace</button>');
        ...
        var deleteWS = $('<button class="btn btn-link pull-right">Delete Workspace</button>');
        ...
        $http.rpc('ws', 'get_workspace_info', {workspace: ws_name}).then(function(info) {
            if (info[5] == 'a') {
                var isAdmin = true;
            } else {
                var isAdmin = false;
            ...
            $('.btn-edit').click(function() {
                if ($(this).hasClass('editable')) {
                    $('.btn-global-perm').html(kbui.globalPermDropDown(perm));
                } else {
            ...
            //if (auth.user ) {
                ...
                var prom = $http.rpc('ws', 'get_permissions', {workspace: ws_name});
        ...
        var prom = $http.rpc('ws', 'get_workspace_description', {workspace:ws_name})
        prom.then(function(descript) {
        ...
        function getPermTable(data) {
            ...
            for (var key in data) {
                ...
                if (key == '*' || key == auth.user || data[key] == 'n') continue;
        ...
        function savePermissions(p_name) {
            ...
            for (var new_user in newPerms) {
                // ignore these
                if (new_user == '*' || new_user == auth.user) continue;
            ...
            for (var user in permData) {
                if (user == '*' || user == auth.user) continue;
